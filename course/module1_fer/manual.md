...
# 下载软件包：
curl -O "${FILE_SERVER_URL}/bricks/module_1_fer/module_a_fer.tar.gz"
# 解压到

sudo mkdir -p /home/embodied/bricks/embodiedapp/module_a_fer/
sudo tar -zxvf module_a_fer.tar.gz -C /home/embodied/bricks/embodiedapp/module_a_fer/
...
![vscode](/resource/oss/download/2082034937892286466)

### 1.7 下载预训练模型权重（线上环境跳过）
...
```bash
# 确保在项目根目录
cd /home/embodied/bricks/embodiedapp/module_a_fer/module_a_fer/models/models

#下载人脸检测insightface的模型

#下载软件包：
curl -O "${FILE_SERVER_URL}/bricks/module_1_fer/buffalo_l.zip"
# 解压到

sudo unzip buffalo_l.zip  -d buffalo_l
...
```python
rom src.models.factory import create_fer_model
...
| POST | `/api/detector/switch` | 切换人脸检测器                       | `{"detector_type": "insightface     |
...
运行效果如下：
![alt text](/resource/oss/download/2082034938211053569)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
<!-- edu-oss-embedded:2082034937892286466,2082034938211053569 -->
