...
```bash
# 下载软件包：
# 首先设置文件服务器的环境变量FILE_SERVER_URL(参考实验环境说明）
export FILE_SERVER_URL="http://192.168.10.47:8088"
# 验证是否生效
echo "${FILE_SERVER_URL}"

# 下载软件包：
curl -O "${FILE_SERVER_URL}/bricks/module_2_behavior/module_b_behavior.tar.gz"

# 解压到

$ sudo mkdir -p /home/embodied/bricks/embodiedapp/module_b_behavior/
$ sudo tar -zxvf module_b_behavior.tar.gz -C /home/embodied/bricks/embodiedapp/module_b_behavior/

# 进入项目根目录
$ cd /home/embodied/bricks/embodiedapp/module_b_behavior/module_b_behavior
...
│   ├── api/              # API 路由和任务管理
│   ├── classroom/        # 室内规则引擎
│   ├── common/           # 公共工具（配置、视频IO、片段聚合）
│   ├── detect/           # 检测封装
│   ├── eval/             # 评估与基准测试
│   ├── extracurricular/  # 室外 MMAction2 识别
│   ├── pipeline/         # 场景路由
│   ├── track/            # 跟踪
│   ├── visualize/        # 可视化绘制
...
├── data/demo/            # 演示图片与视频
├── models/               # 预训练推理权重
│   ├── yolo/             # YOLOv8n / YOLOv8n-Pose
│   └── mmaction2/        # TSN Kinetics-400
├── scripts/              # 辅助脚本（环境检查、benchmark）
├── tests/                # 自动化测试
└── output/               # 运行后生成（标注视频/图片/JSON）
```
...
cd ~
#下载软件包：
curl -O "${FILE_SERVER_URL}/bricks/python_venv/venv.tar.gz"
# 解压到


sudo tar -zxvf venv.tar.gz -C /home/embodied/bricks/embodiedapp/
...
打开 `src/classroom/rules.py`，找到 `pass  # TODO: 请在此补充姿态行为规则引擎代码`，替换为以下代码实现 6 类行为判断：
...
打开 `src/visualize/draw.py`，可以查看绘制的实现方法：
...
打开 `src/track/tracker.py`，找到 `pass  # TODO: 请在此补充 YOLO 检测结果解析代码`，替换为以下代码：
...
打开 `src/common/segments.py`，找到 `pass  # TODO: 请在此补充行为片段聚合代码`，替换为以下代码实现行为片段合并：
...
| `--media`                                   | **必填**，视频路径         |
| `--scene`                                   | `classroom`（默认）        |
| `--max-frames`                              | 限制视频帧数（不填为全部） |
| `--no-vis` / `--no-json` / `--no-keyframes` | 跳过可视化、JSON 或关键帧  |

```bash
python run_infer.py --media data/demo/classroom01_clip_8min_1min.mp4 --output output/classroom01 --scene classroom --max-frames 120
```
...
推理中:   2%|█▍                                                                                            | 2/120 [00:06<06:51,  3.03帧/s, 人=17, 耗时=144ms]
```
...
打开 `src/extracurricular/pipeline.py`，找到 `pass  # TODO: 请在此补充室外流水线单帧处理代码 `，替换为以下代码实现 scene 和 track 两种推理模式：
